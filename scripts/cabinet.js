...
function sum(){//калькулятор
    ...
    if(status==200){
        ...
        if (localStorage.getItem("goodsInBranch")!=undefined){
            ...
            document.getElementById("sum").innerHTML=`There are ${numberItems} items in your branch on sum of ${Math.round(sum*100)/100}$`

        } //если ничего нет
        else{document.getElementById("sum").innerHTML=`There are 0 items in your branch on sum of 0$`}
