if (document.getElementById("catalog")!== null) {
    ...
    if(status==200){
        ...
        for (let i = 0; i < responseObj.size; i+=4) { //загрузка каталога 
...
if (document.getElementById("confirm")!== null) {
    document.getElementById("confirm").addEventListener('click', function(){
        ...
        for(check of checks){if (check.checked==true){l++}}
        let checks2=document.getElementsByClassName("checkCountry")
        for(check of checks2){if (check.checked==true){k++}} //проверка на валидность
        ...
        if (l==0 || k==0) {alert('You have to choose at least one category and one country')}
            ...
            if(status==200){
                ...
                for (let i = 0; i < responseObj.size; i++) {
                    ...
                    if(document.getElementById(String(category)).checked==true &&//проверка на категорию
                    ...
                    ((responseObj.get(String(i+1)).sales!="No" && document.getElementById("Sales").checked==true )
                    ||document.getElementById("Sales").checked==false)){//или фильтр выключен
                        ul.appendChild(li)
                        li.innerHTML=`<img src="${responseObj.get(String(i+1)).src}.png"</img>
                        <div>
                            <h3>${responseObj.get(String(i+1)).name}</h3>
...
if (document.getElementById("catalog") !== null && localStorage.getItem("searchItem")!==null){
    var request = new XMLHttpRequest();
    request.open("GET", "http://localhost:3000/goods", false);
    request.send();
    var status = request.status;
    if(status==200){
        ...
        for (let [key, value] of responseObj) {//если есть совпадение - отрисовка
        ...
        if (ul.innerHTML==""){//если ничего не найдено
            ul.innerHTML=
            `<div>
                <h3>Sorry, we couldn't found anything</h3>
                <h3>Please, make another try</h3>
            </div>`
        }else{//если есть что-то то вызываются листенеры на стрелочки 
            inpytVal();
            clickArrow1();
...
function addBranch1(id){//кнопка хранит айди элемента на добааление 
